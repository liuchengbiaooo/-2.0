...
App({
    ...
    globalData: {
        ...
        ajaxResetUrl: 'https://sl.3xzt.com', //接口请求路径
    ...
    //计算时间
    timeQuantum(time) {
        var beginTime = new Date(time.createTime.replace(/-/g, '/'))
        var finishTime = time.endTime ? new Date(time.endtime.replace(/-/g, '/')) : new Date(); //结束时间


        var separate = finishTime.getTime() - beginTime.getTime() //时间差的毫秒数

        // console.log("separate", separate)
        // console.log("endtime",separate)
        //计算出相差天数
        var days = Math.floor(separate / (24 * 3600 * 1000))

        //计算出小时数
        var leave1 = separate % (24 * 3600 * 1000) //计算天数后剩余的毫秒数
        ...
        if (hours > 1) {
            var beginMinute = hours * 60 + minutes;
        } else if (minutes < 10) {
            var beginMinute = "0" + minutes
        } else if (minutes > 10) {
            var beginMinute = minutes
        }
        if (seconds < 10) {
            var beginSecond = "0" + seconds;
        } else {
            var beginSecond = seconds;
        }
        return { beginMinute, beginSecond }
    },
    //计算时间
    timeQuantumTwo(time, endtime) {
        var beginTime = new Date(time.replace(/-/g, '/'))
        var finishTime = new Date(endtime.replace(/-/g, '/')) //结束时间
        var separate = finishTime.getTime() - beginTime.getTime() //时间差的毫秒数
            //计算出相差天数
        var days = Math.floor(separate / (24 * 3600 * 1000))
